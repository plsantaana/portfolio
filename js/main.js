...
- If your IP address has a poor reputation (often seen with shared VPNs or corporate proxies), try switching to a different, trusted network connection.</pre>`,
    ...
    'multi-step-integration': {
      title: 'Multi- &amp; Alternate Step Integration',
      description: 'Documentation sample from Cloudflare\'s API Shield product covering how to validate API requests against OpenAPI schemas to block malformed or unexpected traffic.',
      detail: [
        '<p><strong>Context:</strong> Documentation sample from Cloudflare\'s API Shield product. The page is written in Visual Studio Code and uses MDX, a Markdown-based authoring format, with custom components.</p>',
        '<div style="display:flex;gap:0;border-bottom:1px solid var(--color-border);margin:1.25rem 0 0;">',
          '<button class="modal-tab-btn active" data-mtab="rendered" style="padding:0.6rem 1.25rem;font-size:0.85rem;font-weight:500;background:transparent;border:none;border-bottom:2px solid var(--color-accent);cursor:pointer;color:var(--color-text);">Rendered</button>',
          '<button class="modal-tab-btn" data-mtab="markdown" style="padding:0.6rem 1.25rem;font-size:0.85rem;font-weight:500;background:transparent;border:none;border-bottom:2px solid transparent;cursor:pointer;color:var(--color-text-muted);">Markdown</button>',
        '</div>',
        '<div class="modal-mtab-content" data-mtab="rendered" style="display:block;">',
          '<div style="border:1px solid var(--color-border);border-radius:0 0 16px 16px;overflow:hidden;max-height:60vh;overflow-y:auto;">',
            '<div style="display:flex;align-items:center;gap:0.75rem;padding:0.75rem 1.25rem;background:var(--color-card-bg);border-bottom:1px solid var(--color-border);font-size:0.8rem;color:var(--color-text-muted);">',
              ...
              '<span style="width:10px;height:10px;border-radius:50%;background:#ffbd2e;display:inline-block;"></span>',
              '<span style="width:10px;height:10px;border-radius:50%;background:#27c93f;display:inline-block;"></span>',
              '<span style="flex:1;padding:0.35rem 0.75rem;background:var(--color-bg);border-radius:6px;font-size:0.8rem;">developers.cloudflare.com / api-shield / security / schema-validation</span>',
            '</div>',
            '<div style="padding:1.5rem 2rem;">',
              '<h2 style="font-size:1.5rem;margin-bottom:0.5rem;">Schema validation</h2>',
              '<p style="color:var(--color-text-muted);margin-bottom:1.5rem;font-size:0.95rem;">Validate API requests against OpenAPI schemas to block malformed or unexpected traffic.</p>',
              '<p style="color:var(--color-text-muted);font-size:0.9rem;line-height:1.7;">The API schema defines which API requests are valid based on several request properties like target endpoint, path or query variable format, and HTTP method.</p>',
              '<p style="color:var(--color-text-muted);font-size:0.9rem;line-height:1.7;">Schema validation allows you to check if incoming traffic complies with a previously supplied API schema. When you provide an API schema or select from a list of learned schemas, API Shield creates rules for incoming traffic from the schema definitions. These rules define which traffic is allowed and which traffic gets logged or blocked.</p>',
              '<div style="background:var(--color-card-bg);border-left:3px solid var(--color-accent);padding:1rem 1.25rem;border-radius:8px;margin:1.5rem 0;font-size:0.9rem;color:var(--color-text-muted);"><strong>Schema validation 2.0</strong><br>Schema validation 2.0 is the current version. For help configuring the previous version, refer to Configure Classic Schema validation. You can migrate to Schema validation 2.0 manually by uploading your schemas to the new system.</div>',
              '<hr style="border:none;border-top:1px solid var(--color-border);margin:1.5rem 0;">',
              '<h3 style="font-size:1.1rem;margin:1.5rem 0 0.75rem;">Process</h3>',
              '<p style="color:var(--color-text-muted);font-size:0.9rem;line-height:1.7;">Endpoints must be added to Endpoint Management for Schema validation to protect them. Uploading a schema via the Cloudflare dashboard will automatically add endpoints, or you can manually add them from API Discovery.</p>',
              '<p style="color:var(--color-text-muted);font-size:0.9rem;line-height:1.7;">The API endpoint is the location where API calls or requests are fulfilled. API Shield defines endpoints as a host, method, and path tuple.</p>',
              '<h3 style="font-size:1.1rem;margin:1.5rem 0 0.75rem;">Add validation by uploading a schema</h3>',
              '<ol style="list-style:none;padding-left:0;margin-bottom:1rem;counter-reset:num;">',
                '<li style="position:relative;padding-left:2rem;margin-bottom:0.3rem;color:var(--color-text-muted);font-size:0.9rem;line-height:1.7;counter-increment:num;"><span style="position:absolute;left:0;font-weight:600;">1.</span> In the Cloudflare dashboard, go to <strong>Security</strong> &gt; <strong>API Shield</strong>.</li>',
                '<li style="position:relative;padding-left:2rem;margin-bottom:0.3rem;color:var(--color-text-muted);font-size:0.9rem;line-height:1.7;counter-increment:num;"><span style="position:absolute;left:0;font-weight:600;">2.</span> Go to <strong>Schema validation</strong> and select <strong>Add validation</strong>.</li>',
                '<li style="position:relative;padding-left:2rem;margin-bottom:0.3rem;color:var(--color-text-muted);font-size:0.9rem;line-height:1.7;counter-increment:num;"><span style="position:absolute;left:0;font-weight:600;">3.</span> Select your schema file for upload.</li>',
                '<li style="position:relative;padding-left:2rem;margin-bottom:0.3rem;color:var(--color-text-muted);font-size:0.9rem;line-height:1.7;counter-increment:num;"><span style="position:absolute;left:0;font-weight:600;">4.</span> Observe the listed endpoints, their host, method, and path. Any new endpoints will automatically be added to Endpoint Management.</li>',
                '<li style="position:relative;padding-left:2rem;margin-bottom:0.3rem;color:var(--color-text-muted);font-size:0.9rem;line-height:1.7;counter-increment:num;"><span style="position:absolute;left:0;font-weight:600;">5.</span> Choose an action for the non-compliant requests to your endpoints.</li>',
                '<li style="position:relative;padding-left:2rem;margin-bottom:0.3rem;color:var(--color-text-muted);font-size:0.9rem;line-height:1.7;counter-increment:num;"><span style="position:absolute;left:0;font-weight:600;">6.</span> Select <strong>Add schema and endpoints</strong>.</li>',
              '</ol>',
              '<h3 style="font-size:1.1rem;margin:1.5rem 0 0.75rem;">Add a fallthrough rule</h3>',
              '<p style="color:var(--color-text-muted);font-size:0.9rem;line-height:1.7;">A fallthrough rule acts as a catch-all for requests that do not match endpoints in Endpoint Management. By ensuring that all your endpoints in a schema are added to Endpoint Management, the fallthrough action can protect you against legacy or zombie endpoints.</p>',
              '<ol style="list-style:none;padding-left:0;margin-bottom:1rem;counter-reset:num2;">',
                '<li style="position:relative;padding-left:2rem;margin-bottom:0.3rem;color:var(--color-text-muted);font-size:0.9rem;line-height:1.7;counter-increment:num2;"><span style="position:absolute;left:0;font-weight:600;">1.</span> Go to <strong>Security</strong> &gt; <strong>API Shield</strong> &gt; <strong>Settings</strong> &gt; <strong>Fallthrough settings</strong>.</li>',
                '<li style="position:relative;padding-left:2rem;margin-bottom:0.3rem;color:var(--color-text-muted);font-size:0.9rem;line-height:1.7;counter-increment:num2;"><span style="position:absolute;left:0;font-weight:600;">2.</span> Select <strong>Use Template</strong>.</li>',
                '<li style="position:relative;padding-left:2rem;margin-bottom:0.3rem;color:var(--color-text-muted);font-size:0.9rem;line-height:1.7;counter-increment:num2;"><span style="position:absolute;left:0;font-weight:600;">3.</span> Choose one or more hostnames from the dropdown menu.</li>',
                '<li style="position:relative;padding-left:2rem;margin-bottom:0.3rem;color:var(--color-text-muted);font-size:0.9rem;line-height:1.7;counter-increment:num2;"><span style="position:absolute;left:0;font-weight:600;">4.</span> Name your rule and select your action.</li>',
                '<li style="position:relative;padding-left:2rem;margin-bottom:0.3rem;color:var(--color-text-muted);font-size:0.9rem;line-height:1.7;counter-increment:num2;"><span style="position:absolute;left:0;font-weight:600;">5.</span> Select <strong>Deploy</strong>.</li>',
              '</ol>',
              '<h3 style="font-size:1.1rem;margin:1.5rem 0 0.75rem;">Managing actions</h3>',
              '<p style="color:var(--color-text-muted);font-size:0.9rem;line-height:1.7;">Schema validation\'s default action applies to any endpoint with its action set to <code style="background:var(--color-card-bg);padding:0.15rem 0.4rem;border-radius:4px;">Default</code>. Available actions:</p>',
              '<ul style="list-style:none;padding-left:0;margin-bottom:1rem;">',
                '<li style="position:relative;padding-left:1.4rem;margin-bottom:0.3rem;color:var(--color-text-muted);font-size:0.9rem;line-height:1.7;">&mdash; <strong><code style="background:var(--color-card-bg);padding:0.15rem 0.4rem;border-radius:4px;">Log</code></strong> &mdash; logs events to Firewall Events.</li>',
                '<li style="position:relative;padding-left:1.4rem;margin-bottom:0.3rem;color:var(--color-text-muted);font-size:0.9rem;line-height:1.7;">&mdash; <strong><code style="background:var(--color-card-bg);padding:0.15rem 0.4rem;border-radius:4px;">Block</code></strong> &mdash; blocks requests that fail the schema and logs events.</li>',
                '<li style="position:relative;padding-left:1.4rem;margin-bottom:0.3rem;color:var(--color-text-muted);font-size:0.9rem;line-height:1.7;">&mdash; <strong><code style="background:var(--color-card-bg);padding:0.15rem 0.4rem;border-radius:4px;">None</code></strong> &mdash; non-compliant requests are neither logged nor blocked.</li>',
              '</ul>',
              '<h3 style="font-size:1.1rem;margin:1.5rem 0 0.75rem;">Specifications &amp; Limitations</h3>',
              '<p style="color:var(--color-text-muted);font-size:0.9rem;line-height:1.7;">Cloudflare currently only accepts <strong>OpenAPI v3 schemas</strong>. The accepted file formats are YAML (<code>.yml</code> / <code>.yaml</code>) and JSON (<code>.json</code>). OAS v3.1 is not supported.</p>',
              '<table style="width:100%;border-collapse:collapse;margin:1rem 0;font-size:0.9rem;">',
                '<thead><tr><th style="border:1px solid var(--color-border);padding:0.6rem 1rem;text-align:left;background:var(--color-card-bg);font-weight:600;color:var(--color-text);">Plan</th><th style="border:1px solid var(--color-border);padding:0.6rem 1rem;text-align:left;background:var(--color-card-bg);font-weight:600;color:var(--color-text);">Body size limit</th></tr></thead>',
                '<tbody>',
                  '<tr><td style="border:1px solid var(--color-border);padding:0.6rem 1rem;color:var(--color-text-muted);">Free</td><td style="border:1px solid var(--color-border);padding:0.6rem 1rem;color:var(--color-text-muted);">1 KB</td></tr>',
                  '<tr><td style="border:1px solid var(--color-border);padding:0.6rem 1rem;color:var(--color-text-muted);">Pro</td><td style="border:1px solid var(--color-border);padding:0.6rem 1rem;color:var(--color-text-muted);">8 KB</td></tr>',
                  '<tr><td style="border:1px solid var(--color-border);padding:0.6rem 1rem;color:var(--color-text-muted);">Business</td><td style="border:1px solid var(--color-border);padding:0.6rem 1rem;color:var(--color-text-muted);">8 KB</td></tr>',
                  '<tr><td style="border:1px solid var(--color-border);padding:0.6rem 1rem;color:var(--color-text-muted);">Enterprise</td><td style="border:1px solid var(--color-border);padding:0.6rem 1rem;color:var(--color-text-muted);">128 KB</td></tr>',
                '</tbody>',
              '</table>',
              '<h3 style="font-size:1.1rem;margin:1.5rem 0 0.75rem;">Troubleshooting: OneOf constraint error</h3>',
              '<p style="color:var(--color-text-muted);font-size:0.9rem;line-height:1.7;">A <code style="background:var(--color-card-bg);padding:0.15rem 0.4rem;border-radius:4px;">OneOf</code> constraint error means a request body did not match exactly one option in a <code>oneOf</code> list. The request either matched zero subschemas (missing required fields) or matched multiple (ambiguous payload). Check the failing request body against the API schema definition to resolve.</p>',
            '</div>',
          '</div>',
        '</div>',
        '<div class="modal-mtab-content" data-mtab="markdown" style="display:none;">',
          '<div style="border:1px solid var(--color-border);border-radius:0 0 16px 16px;overflow:hidden;max-height:60vh;overflow-y:auto;">',
            '<pre style="margin:0;padding:1.25rem 1.5rem;background:#1a1a1a;color:#e0e0e0;font-family:SF Mono,Fira Code,Cascadia Code,Consolas,monospace;font-size:0.8rem;line-height:1.7;overflow-x:auto;tab-size:2;">',
'---\ntitle: Schema validation\ncontent_type: concept\ndescription: Validate API requests against OpenAPI schemas to block malformed or unexpected traffic.\nproducts:\n  - api-shield\nsidebar:\n  order: 3\n---\n\nimport { Render, GlossaryTooltip } from "~/components";\n\nThe API schema defines which API requests are valid based on several request properties like target endpoint, path or query variable format, and HTTP method.\n\nSchema validation allows you to check if incoming traffic complies with a previously supplied API schema. When you provide an API schema or select from a list of learned schemas, API Shield creates rules for incoming traffic from the schema definitions. These rules define which traffic is allowed and which traffic gets logged or blocked.\n\n:::note[Schema validation 2.0]\nSchema validation 2.0 is the current version. For help configuring the previous version, refer to [Configure Classic Schema validation](/api-shield/reference/classic-schema-validation/).\n:::\n\n---\n\n## Process\n\nEndpoints must be added to [Endpoint Management](/api-shield/management-and-monitoring/endpoint-management/) for Schema validation to protect them. Uploading a schema via the Cloudflare dashboard will automatically add endpoints, or you can manually add them from [API Discovery](/api-shield/security/api-discovery/).\n\nThe API endpoint is the location where API calls or requests are fulfilled. API Shield defines endpoints as a host, method, and path tuple.\n\n## Adding validation\n\n### Upload a schema\n\n1. In the Cloudflare dashboard, go to **Security** > **API Shield**.\n2. Go to **Schema validation** and select **Add validation**.\n3. Select your schema file for upload.\n4. Observe the listed endpoints, their host, method, and path.\n5. Choose an action for the non-compliant requests.\n6. Select **Add schema and endpoints**.\n\n### Add a fallthrough rule\n\nA fallthrough rule acts as a catch-all for requests that do not match endpoints in [Endpoint Management](/api-shield/management-and-monitoring/endpoint-management/).\n\n1. Go to **Security** > **API Shield** > **Settings** > **Fallthrough settings**.\n2. Select **Use Template**.\n3. Choose one or more hostnames from the dropdown menu.\n4. Select **Continue to custom rule**.\n5. Name your rule and select your action.\n6. Select **Deploy**.\n\n## Managing actions\n\nSchema validation\'s default action applies to any endpoint with its action set to `Default`:\n\n- `Log` — logs events to [Firewall Events](/firewall/).\n- `Block` — blocks requests that fail the schema and logs events.\n- `None` — non-compliant requests are neither logged nor blocked.\n\n## Specifications & Limitations\n\nCloudflare currently only accepts [OpenAPI v3 schemas](https://spec.openapis.org/oas/v3.0.3.html). Accepted file formats are YAML (`.yml` / `.yaml`) and JSON (`.json`). OAS v3.1 is not supported.\n\n### Body size for validation\n\n| Plan       | Body size limit |\n| --- | --- |\n| Free       | 1 KB            |\n| Pro        | 8 KB            |\n| Business   | 8 KB            |\n| Enterprise | 128 KB          |\n\nRequired fields: `schema.type` (all schemas require a type) and `parameter.schema` (the content field is not supported in parameters).\n\n## Troubleshooting\n\n### `OneOf` constraint error\n\nA `OneOf` constraint error means a request body did not match exactly one of the options in a `oneOf` list within your schema. The request was invalid because it matched zero subschemas (missing required fields) or matched multiple subschemas (ambiguous payload). Check the failing request body against your API schema definition.',
            '</pre>',
          '</div>',
        '</div>'
      ].join('')
